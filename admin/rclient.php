...
<div align="center">
	<div class="main_div">
		<div class="main">
			<div class="main_site">
			  <div class="header">Report on all users created<br />
...
{
	  $result ="";
	 $result = mysql_query("SELECT * FROM users where date>='$date' and date<='$date1' order by date ")or die(mysql_query());
...
echo "<tr><td>{$row['name']}</td><td>{$row['surname']}</td><td>{$row['username']}</td><td>{$row['email']}</td><td>{$row['date']}</td></tr>";
}
}
